 ...
 *
 */
public class DefaultImageSplitter implements DataSplitter<inputDataSplit,ImageConfiguration> {
	...
	@Override
	public inputDataSplit getDataSplit(Path path,ImageConfiguration configuration) {
		 ...
		 return new inputDataSplit(trainData,labelGenerator);
	}
	...
	@Override
	public inputDataSplit getEvalImageSplit(Path path, ImageConfiguration configuration) {
		 File mainPath = path.toFile();			
		 FileSplit fileSplit = new FileSplit(mainPath, NativeImageLoader.ALLOWED_FORMATS, getRandom());		 
